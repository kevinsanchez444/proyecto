...
ngOnInit() {
 ...
 verProductos() {
  this.carritoServicio.obtenerProductos().subscribe((datos: Carrito[]) => {
    ...
    console.error('❌ Error al obtener productos del carrito:', error); // ✅ VERIFICACIÓN 2
  });
}
...
actualizarCantidad(idCarrito: number, cantidad: number): void {
  if (!cantidad || cantidad <= 0) return; // No hace nada si la cantidad es inválida
  ...
  this.carritoServicio.vaciarCarrito(cedula).subscribe(dato => {
    try {
      if (dato != false) {
        ...
        this.ngOnInit(); // Recargar los productos del carrito
      } else {
...
onGeneratePDF() {
  ...
  const reciboNo = Math.floor(Math.random() * 100000).toString(); // Puedes generar uno aleatorio o usar uno real
